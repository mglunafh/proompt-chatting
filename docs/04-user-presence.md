...
- **No self-echo** — a user's own sessions do not receive their own presence events.
- **Offline is broadcast after a ~20s grace period**, cancelled by a reconnect inside the window, so a network blip does not flicker every roster. Detection is already delayed by the heartbeat timeout; the grace makes the delay deliberate rather than incidental.

## Snapshot at connect

- **Pushed as the first frame after upgrade** — the registry operation that adds the socket captures the map in the same step, so nothing can change between the snapshot and the first delta. Fetching over REST instead would leave a gap in which a change reaches every connected socket but not the one still registering, and the client would render that user wrong until their next transition, with neither side able to detect it.
...
- **Carried on presence events and in the snapshot** — a change is broadcast on the same channel and to the same audience as a presence change, and the roster pushed at connect includes each user's status, so a client is never left waiting for someone to change it.
...
- **Not hideable** — no per-user opt-out.
...
- **Re-asserted about every 4s while typing** — timer-driven rather than keystroke-driven: emit on the first keystroke, then at most once per interval while the input is non-empty and has changed since the last tick, so a fast typist and a slow one produce the same traffic.
...
- **Floored server-side** — frames arriving faster than roughly one per two seconds per conversation are dropped rather than counted against the interaction rate limit, so a misbehaving client cannot burn a real user's send budget with typing spam.
