...
- **Conversation metadata rides its own event** — a rename, a description edit, or an archive or unarchive is pushed to the members as a typed frame rather than a message, so it stays out of history, out of unread counts, and out of the posts-only tally a channel keeps. A client that was offline for it learns the new value from the conversation list it refetches on reconnect.
