...
- **A DM is a `direct` conversation with two members** — unique partial index over the canonical pair (`least(a, b), greatest(a, b)`), or concurrent first-sends fork the history.
- **DM conversations are created implicitly on first send** — no separate "open chat" call.
- **Self-DMs are allowed** — the canonical pair permits `a = b`.
...
- **Every message has a sender** — `sender_id` is `NOT NULL`. Server-generated notices ride typed events instead of message rows, so no read, edit, delete, pin, reaction or unread rule needs a sender-less case, and nothing renders without an author.
...
- **Conversation metadata rides its own event** — a rename or description edit is pushed to the members as a typed frame rather than a message, so it stays out of history, out of unread counts, and out of the posts-only tally a channel keeps. A client that was offline for it learns the new value from the conversation list it refetches on reconnect.
- **Membership changes ride their own event** — a join or a leave is pushed to the members as a typed frame and never persisted as a message. Live signal only: an offline client sees the resulting roster when it refetches, and there is no record in the conversation of who joined when. Buying that record back means a second history path, not a nullable `sender_id`.
...
- **A cursor advance is fanned out to the user's own sessions** — the cursor is shared across them, so reading in one clears the unread state in the rest. The only fan-out aimed at a single user rather than at a conversation's members.
- **Mentions are stored as rows** — `(message_id, user_id)`, written in the transaction that inserts the message, so the mention tally in [notes-notifications.md](notes-notifications.md) can be aggregated at query time rather than parsed out of bodies. True of either derivation below. Undecided is how the set is derived:
  - **The server parses `@username`** — nothing new on the wire, mention rows resolved at send. A rename leaves old messages showing stale text, and an email address or a code snippet can parse as a mention.
  - **The client sends ranges** — `(start, length, userId)` beside a body stored exactly as typed, offsets in UTF-16 code units. Renames render correctly and the server never scans text, at the cost of validating entities, a name resolver in the client, and re-deriving the set on every edit.
