...
import dev.burufi.chatting.durable.shared.TestCharacters.BIDI_OVERRIDE
import dev.burufi.chatting.durable.shared.TestCharacters.CSI
import dev.burufi.chatting.durable.shared.TestCharacters.DEL
...
import dev.burufi.chatting.durable.shared.TestCharacters.HIGH_SURROGATE
import dev.burufi.chatting.durable.shared.TestCharacters.LOW_SURROGATE
import dev.burufi.chatting.durable.shared.TestCharacters.NUL
import dev.burufi.chatting.durable.shared.TestCharacters.ROCKET
import dev.burufi.chatting.durable.shared.TestCharacters.ZERO_WIDTH
    ...
    companion object {
        ...
        fun accepted(): List<Arguments> =
            listOf(
                ...
                case(
                    ...
                    """quote " backslash \ $E_ACUTE""",
                    """quote " backslash \ $E_ACUTE""",
                ),
                case("a surrogate pair survives whole", "a${ROCKET}b", "a${ROCKET}b"),
                case(
                    "a bidi override is the client's to render, not a rule here",
                    "a${BIDI_OVERRIDE}b",
                    "a${BIDI_OVERRIDE}b",
                ),
        ...
        fun rejected(): List<Arguments> =
            listOf(
                ...
                case(
                    "the scan runs before the line cap",
                    "$ESC" + "\n".repeat(200),
