...
## Progress names the file the page reads at open to mark done work; only the
name is carried into deps.html.
...
<!-- The file the page reads when it opens, to tick what is already done.
     Omit the section for __progress.md beside the page; include it
     with nothing listed to mark nothing. -->
## Progress
...
def generated_note(path: Path, stamp: bool) -> str:
    """The line both outputs open with, marking them as artifacts of the deps file."""
    when = f" on {datetime.now():%Y-%m-%d %H:%M}" if stamp else ""
    return f"Generated by deps-graph.py from {path.name}{when}"


def progress_paths(graph: Graph) -> list[str]:
    """What ## Progress names, passed through untouched for the page to fetch.

    Resolved by the browser against the page, not by us against the build
    machine, so a missing one is not checked for here: with --out-dir the two
    directories differ, and the build machine's layout would be the wrong
    thing to answer from.
    """
    if graph.progress_declared:
        return list(graph.progress)
    return list(PROGRESS_FILES)


def render_html(graph: Graph, template: str, note: str) -> str:
    data = {
        "nodes": [
    ...
    return page.replace("__GENERATED__", html.escape(note)) + "\n"
...
def render_mermaid(graph: Graph, note: str) -> str:
    lines = [f"# {graph.title}", "", f"*{note}*", "", "## By depth", ""]
...
def run(
    ...
    for path in paths:
        ...
        note = generated_note(path, stamp)

        try:
            template = template_path.read_text(encoding="utf-8")
            page = render_html(graph, template, note)
        ...
        mermaid_path.write_text(render_mermaid(graph, note), encoding="utf-8")
...
sections of a deps.md file:
  ...
  ## Progress   path/to/progress.md  what the page reads when it opens, to
                  tick finished work (default: {', '.join(PROGRESS_FILES)})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-timestamp",
        action="store_true",
        help="build: leave the time out of the generated-by line, for reproducible output",
    )
    parser.add_argument(
        "--force",
        action="store_true",
